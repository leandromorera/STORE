...
def main():
    ...
    * { box-sizing: border-box; }
    ...
    print("\\nCreate a user (quick):")
    print("  python - <<'PY'")
    print("import os,bcrypt")
    print("from sqlalchemy import create_engine,select")
    print("from sqlalchemy.orm import Session")
    print("from models import Base,User,Device")
    print("from dotenv import load_dotenv")
    print("load_dotenv(); engine=create_engine(os.getenv('DATABASE_URL','sqlite:///ecommerce.db'),future=True)")
    print("Base.metadata.create_all(engine)")
    print("email='you@example.com'; pw='changeme'; device='e356c8c1ee8ca937'")
    print("with Session(engine) as db:")
    print("  u=db.execute(select(User).where(User.email==email)).scalar_one_or_none()")
    print("  if not u:")
    print("    u=User(email=email,password_hash=bcrypt.hashpw(pw.encode(),bcrypt.gensalt()).decode()); db.add(u); db.flush();")
    print("    db.add(Device(device_id=device,user_id=u.id)); db.commit(); print('created user & device')")
    print("  else:")
